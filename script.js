...
// Per-page settings. Each invitation page sets window.INVITE before this file
// loads: { base: "../", countdown: "2026-11-27T16:00:00+05:00" }.
const CFG = window.INVITE || {};
const BASE = CFG.base || "";
// The countdown on the opening page, in Pakistan time (UTC+5).
...
function preloadEverything() {
  ...
  // the envelope pieces are always there; the cards are whatever this page holds
  const art = ["env-body.png", "env-flap.png", "env-flap-lining.png", "seal.png", "dawat-nama.png"]
    .map((f) => BASE + f)
    .concat(Array.from(document.querySelectorAll("img[data-src]")).map((i) => i.dataset.src));
...
function playClosing() {
  const scene = document.querySelector(".scene-close");
  if (!scene) return;
...
/* ---------- which page are we on, by name not by number ---------- */
const PAGE_NAMES = ["open", "mehndi", "baraat", "walima", "close"];
function pageNameOf(scene) {
  if (!scene) return "open";
  return PAGE_NAMES.find((n) => scene.classList.contains("scene-" + n)) || "open";
}
...
function goTo(n, { instant = false } = {}) {
  ...
  document.body.dataset.page = pageNameOf(scenes[target - 1]);
...
document.body.dataset.page = pageNameOf(scenes[0]);
